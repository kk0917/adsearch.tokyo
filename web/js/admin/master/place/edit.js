...
(function () {
  ...
  document.getElementById('placeEditArea').addEventListener('click', function(event) {
    if (event.target.getAttribute('data-toggle') == 'modal') {
      window.setTimeout(function () { // オーバーレイ要素が</body>直前に追加されるのを待つ
        ...
        var modalWindow = document.querySelectorAll('#placeEditArea .modal'); // モーダル画面要素は増減しない
  ...
  $('#placeEditConfirmModal').on('hidden.bs.modal', function () {
    $('body').addClass('modal-open');
  });

  // メイン画像・サブ画像選択時のプレビュー表示
  ...
  document.getElementById('execUpdate').addEventListener('click', function() {
    var updatePlace = document.editPlaceForm;
    var sendData = new FormData(updatePlace);

    var ajax = new XMLHttpRequest();
    ajax.open('POST', '/admin/master/place/edit.php', true);
    ...
    function execUpdate() {
      if(ajax.readyState === XMLHttpRequest.DONE) {
        if (ajax.status === 200) {
          ...
          var errorModalBody = document.querySelector('#placeEditValidationErrorModal .modal-body');
